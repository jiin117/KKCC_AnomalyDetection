...
추론 실행 시간(초) 및 GPU 메모리 소비량(MiB)을 측정하는 스크립트입니다.
참고: 본 스크립트는 CPU 환경에서 모델을 구동할 경우의 메모리 소비량 측정은 지원하지 않습니다.

General usage (일반적인 사용법):
- Specify OUT_DIR: 결과가 저장될 출력 디렉터리를 지정하십시오.
- Insert your model and forward pass call: 모델 정의 및 순전파 호출 코드를 삽입하십시오. (# INSERT 표시 확인)
- Run this script: 이 스크립트를 실행하십시오.
"""
