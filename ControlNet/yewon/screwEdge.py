###############################################################################################
'''
자동 밝기 조절 후 외곽선 따는 코드 (표시해둔 부분 복사 후 사용)

· 사진의 평균 빛 밝기를 계산 / 스스로 밝기 조절 작업 후 외곽선을 따주는 코드 추가해봤습니다.
·  색상과 톤을 균일하게 하기 위해 이미지를 먼저 흑백으로 처리 후 외곽선 땄습니다. 
· 저는 이걸 cable에다 먹이니 결과가 그리 좋진 못했지만 다른 데이터에서는 효과가 좋을 것 같아 올립니다.
· 주의!!! cable 폴더의 10개 데이터에만 적용해본다고 dataset_path, outpath 등 경로를 만졌습니다. 표시해둔 부분만 복사하여 본인의 코드에 넣으시고 적절히 고쳐 사용 바랍니다.
'''
#################################################################################################
...
# cv2.imwrite("output_edge.png", result_image)
